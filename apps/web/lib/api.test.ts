...
describe('avenueApiUrl env resolution', () => {
  ...
  it('ignores relative NEXT_PUBLIC_API_BASE_URL in server routes', async () => {
    vi.stubEnv('AVENUE_API_URL', '')
    vi.stubEnv('NEXT_PUBLIC_API_BASE_URL', '/api')
    const { avenueApiUrl } = await import('./api')
    expect(avenueApiUrl('/auth/register')).toBe('http://localhost:4000/auth/register')
  })

  it('normalizes trailing slash in API base URL', async () => {
    vi.stubEnv('AVENUE_API_URL', 'https://server-only.example.com/')
